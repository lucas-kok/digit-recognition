...
class LetterRecognizerApp:
    def __init__(self, root, Theta1, Theta2, Theta3):
        ...
        # Title
        self.title_label = ttk.Label(root, text="Draw a Letter and Click Predict", font=('Helvetica', 16))
        ...
        # Predict button
        self.predict_button = ttk.Button(self.button_frame, text="Predict", command=self.predict_letter)
        ...
        self.categories = [chr(i) for i in range(65, 65 + 26)]  # A-Z
    ...
    def predict_letter(self):
        ...
        self.result_label.config(text=f"Predicted Letter: {self.categories[prediction[0]]}")

if __name__ == "__main__":
    Theta1 = np.loadtxt('model/letters/Theta1.txt')
    Theta2 = np.loadtxt('model/letters/Theta2.txt')
    Theta3 = np.loadtxt('model/letters/Theta3.txt')
    ...
    app = LetterRecognizerApp(root, Theta1, Theta2, Theta3)
    root.mainloop()
